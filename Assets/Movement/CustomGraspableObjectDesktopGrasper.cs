...
namespace Ubiq.XR
{
    ...
    /// </summary>
    [RequireComponent(typeof(CustomDesktopHand))]
    public class CustomGraspableObjectDesktopGrasper : MonoBehaviour
    {
        ...
        private CustomDesktopHand hand;

        private void Awake()
        {
            hand = GetComponent<CustomDesktopHand>();
        }
        ...
        private void UpdateGrasp()
        {
            ...
            if (Input.GetMouseButtonDown(mouseButton) && grasped == null)
            {
                if (Input.GetMouseButtonDown(mouseButton)) // test if there is a graspable object under the cursor on this click. todo: key modifider for mac
                {
                    ...
                    if (UnityEngine.Physics.Raycast(mainCamera.ScreenPointToRay(Input.mousePosition), out hit, 100,
